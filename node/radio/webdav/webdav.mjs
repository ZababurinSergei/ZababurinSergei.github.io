            ...
            case '/storage/audio/streem':
                try {
                    if (await storage.staticProperty.client.exists("/audio-streem") === false) {
                        ...
                        try{
                            // let stream = fs.createReadStream(`${__dirname}/docs/streem/test.mp3`)    
                            out(stream)
                        }catch (e) {
                            out({_:'error', error: e})
                        }
                        ...
                        try{
                            let stat = await storage.staticProperty.client.stat("/audio-streem/test.mp3")
                            console.log('stat------',stat)
                            let stream = storage.staticProperty.client.createReadStream("/audio-streem/test.mp3")
                            // let stream = fs.createReadStream(`${__dirname}/docs/streem/test222.mp3`).pipe(storage.staticProperty.client.createWriteStream("/audio-streem/testffff.mp3"))
                            out(stream.pipe(throttle))
                        }catch (e) {
                            out({_:'error', error: e})
                        }
                    }
