...
export default async (self) => {
    ...
    const container = self.querySelector('.term')
    container.innerHTML = ''
    term.open(container);
    ...
    // fitAddon.fit();
    task.get(true, 'await', '5', '','/fitAddon/fit/', async (event) => {
        event.callback(fitAddon.fit())
    })
...
//   `);
    ...
    const module = WebAssembly.compileStreaming(fetch('/dex/components/terminal/coreutils.async.wasm'));

    const currentlyDefined = async () => {
        let helpStr = '';
        await new Bindings({
